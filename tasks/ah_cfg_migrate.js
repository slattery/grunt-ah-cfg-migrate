		...
		files.forEach(function(file) {
			...
			var stats = fs.statSync(srcfile);
			 if (stats.size > 0) {	
					...
					var envdstdir 
						= path.normalize(process.cwd() + c.migrateconfigs.configs.dstprefix);
					...
					input = falafel(srctext, function (node) {
						if ( 
							...
							env_tokens.push( { "token" : tok, "payload" : str } );
  				...
  				for(var i = 0, limit = env_tokens.length; (i < limit); i++){
						...
						if (grunt.file.exists(envdst)){
							...
							if ( !processed ){
								...
								grunt.log.writeln('File ' 
									+ chalk.blue(file) 
									+ ' is not there, copying src file to destination');
